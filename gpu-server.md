...
Note that running `watch -n0.1 nvidia-smi` leads to significantly slower distributed training: there is a 15% performance drop when all four GPUs are running at full tilt (although only a ~5% drop for one GPU). This is a much more noticeable drop than for consumer grade hardware (RTX 3060 with i7-12700F) and it is unknown why it occurs. A similar drop in performance is seen for SXM4 A100 clusters with NVLink during training runs with related distributed methods such as FSDP, suggesting that this is not a problem specific to the V100 cluster but is a more general phenomenon. A simple test to see if this was caused by nccl calls in the NVLink system would be to remove all GPUs save one and repeat the same training runs with `nvidia-smi` running concurrently, but I will not be doing this as removing and replacing SXM2 modules is a pain. 
