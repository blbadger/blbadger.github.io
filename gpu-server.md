...
**Inferencing Large Models**: Mostly overkill. The ability to run 70 billion parameter models with mild quantization very quickly (16 t/s) is quite nice, but even here the GPUs experience low utilization. For batched inference or if multiple models are split between GPUs then the they are better-utilized, but otherwise you are probably better off with 3090s or L40s if price is not an object. If it is, this is still the best deal.
