...
Probability theory usually assumes independent events.  An assumption of independent events implies additivity, meaning that if we know the probability of one event and the probability of another event then we can know the probability of both occuring.  Being that these are restrictive assumptions that often do not seem to reflect the natural world, a different approach can be made.  
