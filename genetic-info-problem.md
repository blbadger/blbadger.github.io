...
Second, protein folding is sensitive to initial conditions. A chemically significant (nonpolar to ionic, for example) change in amino acid quality may be enough to drastically change the protein's final shape, an example of which is the E6V (charged glutamic acid to nonpolar valine) mutation of Hemoglobin. This is not surpising, given that a protein with a modest number of 101 amino acids has $3^{100}$ possible likely configurations (3 chemically probable orientations for 100 moveable bonds in a chain) [ref](https://www.pnas.org/content/pnas/89/1/20.full.pdf).  
