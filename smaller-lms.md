...
Seeking to make a more efficient learning algorithm than a transformer, we used the observation that input representation is superior for modified MLP-Mixer architectures to craft a model capable of replicating the autoregressive language generation of GPT-style decoder-only transformers. 

We have seen that masked mixers are typically more efficient learners than transformers for the task of causal language modeling on TinyStories with no changes to the default architecture.  Mixers are typically competetive with or slightly wores than hyperparameter-optimized transformers on newer hardware with key, query, and value optimizations like Flash Attention 2, depending on the hardware. Masked mixers are, as implemeted, less efficient for inference because they are highly parallelized and have a fixed-size context window for each forward pass.

On the other hand, masked mixers are far more effective at retrieval tasks, as evidenced by the observation that a summary-story matching retrieval model is capable of far better training on embeddings from these models compared to embeddings from transformers. For this task, inference would be approximately identical as one typically loads full context for each forward pass for retrieval tasks.
...
2. Depending on the hardware the models are implemented on, given equal compute a masked mixer may reach a much lower training and validation causal language model loss or else a modern, dataset-optimized transformer may out-perform the mixer.
...
4. Compared to the transformer models as originally introduced or adapted for CLM, masked mixers are more efficient learning algorithms. It is only with the optimizations of recent years (RoPE, FA2 etc) that the transformer becomes competitive for causal language modeling.
5. As mixers exhibit far more accurate input representations and thus contain more information on the input than transformers, one expects better sequence-to-sequence retrieval performance from these models. This is exactly what we find when training a model on mixer versus transformer embeddings.
...
To conclude, the masked mixer architecture is introduced, motivated by superior input representation accuracy in this model compared to the transformer. This mixer architecture is in many ways much simpler than the transformer, but performs suprisingly well considering the many optimizations introduced since the first transformer architecture in 2017.  With default parameters the masked mixer outperforms the transformer in terms of training efficiency, and after extensive optimization of architecture and hyperparameters it performs approximately on par with or slightly worse than the transformer for causal language training tasks. Transformer-mixer hybrids offer the most efficient models observed for this dataset, outperforming Llama style models with many of the latest optimizations (SPDA attention, Flash attention 2, RoPE etc.).

These results indicate that a highly optimized transformer with its intrinsic focus on relatively few input elements for each output results in approximately as or slightly more efficient a learning paradigm as the masked mixer's lack of focus if the transformer is properly tuned, depending on the hardware. On the other hand, comparing transformer architectures without many of these optimizations (with models typical of those introduced in 2017, 2018, and 2019) we see that the masked mixer is a far more efficient learning algorithm, calling into question the notion that superior representational accuracy is not as important as superior representational power.

It is clear that accurate input representation is expected to and does indeed correlate with much better retrieval model performance. 
