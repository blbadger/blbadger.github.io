...
4. As mixers exhibit far more accurate input representations and thus contain more information on the input than transformers, one expects better sequence-to-sequence retrieval performance from these models. This is exactly what we find when training a model on mixer versus transformer embeddings.
...
To conclude, a new model architecture is introduced which is motivated by superior input representation accuracy. This model architecture is in many ways much simpler than the transformer, but performs suprisingly well considering the many optimizations introduced since the first transformer architecture in 2017.  With default parameters the masked mixer outperforms the transformer in terms of training efficiency, and after extensive optimization of architecture and hyperparameters it performs approximately on par with or slightly worse than the transformer for causal language training tasks. Transformer-mixer hybrids offer the most efficient models observed for this dataset.

These results indicate that the transformer's intrinsic focus on relatively few input elements for each output results in approximately as efficient a learning paradigm as the mixer's lack of focus (and better input representation) if the transformer is properly tuned, but that this architecture is highly sensitive to parameters like head number, hidden dimension, and learning rates. 

On the other hand, accurate input representation was expected to and does indeed correlate with much better retrieval model performance.
