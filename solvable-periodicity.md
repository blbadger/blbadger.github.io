...
Take the mapping of Turing machine (or $\lambda$ -recursive function, as in Church's original thesis) inputs to the outputs as a dynamical system, where each element of the set of countably infinite inputs $\left i_0, i_1, i_2 ... \right$ is mapped to its output $ \left O_0, O_1, O_2 ... \right$ as 
...
As the inputs are countable, they can be arranged in sequence.  Taking the sequence of decision procedures $\mathscr D$ for all inputs, the function mapping an input $\left i_0, i_1, i_2 ... \right$ to its decision procedure $ \left \mathscr D_0, \mathscr D_1, \mathscr D_2 \right$ defined as $f(i)$ can be viewed as a discrete dynamical system.  For all decidable problems, this functions is periodic for soome ordering of $i$.
