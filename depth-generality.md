...
It has previously been observed that convolutional weights in early layers tend to make edge detectors, which has been understood to signify the ifrst step in the abstraction that follows in later layers (in which entire objects are detected).  There is another possibility, however: the early layers learn convolutions that serve to restrict the respresentation of the input in that layer, ie they sharpen that representation to become more like the input in certain ways.
