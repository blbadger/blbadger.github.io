...
It has previously been observed that convolutional weights in early layers tend to make edge detectors, which has been understood to signify the first step in the abstraction that follows in later layers (in which entire objects are detected).  There is another possibility, however: the early layers learn convolutions that serve to restrict the possible inputs that could yield a similar representation in that layer, which allows that representation to become visually sharpened and more clear relative to the untrained case.
